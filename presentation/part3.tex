...
\begin{frame}[fragile]{The Problem}
    ...
    \textbf{The Context}:
    \begin{itemize}
        \item The landscape of tabular data generation is currently dominated by deep generative models. Unconditional sampling is often the only exact probabilistic query they can compute.
...
\begin{frame}[fragile]{Structure and Tractability}
    ...
    \begin{columns}
       ...
       \begin{itemize}
            ...
            \item Only multiply functions defined on disjoint sets of variables. Ex: $p(x_1, x_2) = a(x_1) b(x_2)$

            \item Only sum functions defined over identical sets of variables. Ex: $p(x_1, x_2) = w_1 a(x_1,x_2) + w_2 b(x_1, x_2)$
...
\begin{frame}[fragile]{Tractable Queries}
    ...
    Probabilistic circuits satisfying these properties are \textbf{hierarchical mixtures} allowing the following inference queries to be computed in time linear in the size of the circuit:
    \begin{itemize}
...
\begin{frame}[fragile]{TabPC}
    ...
    We call TabPC the overparametereized PC we use for tabular data generation, characterized the following design choices:
    \begin{itemize}
        ...
        \item + Preprocessing for tabular data generation.
    \end{itemize}
...
\begin{frame}[fragile]{Example}
    ...
    Example of TabPC circuit built for the Magic dataset (12 features/columns).
    \begin{figure}
        \centering
...
\begin{frame}[fragile]{Experiments}
    ...
    We aim at comparing with state-of-the-art diffusion-based methods for tabular data generation. We run the experiments in the same settins as TabDiff\citep{tabdiff}:
