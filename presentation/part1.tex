...
\frametitle{Controlling Generative Models}
    ...
    A relevant problem in generative models is to efficiently achieve conditional generation by forcing user-defined logical constraints on generated samples.

    \begin{itemize}
        \item Example: \textit{"Given a generative model for tabular data with 3 fields $p(x_1,x_2,x_3)$, sample from $p(x_1, x_2, x_3 \mid x_1>x_2 \land x_3 = 2)$"}
        \item Example: \textit{"Given a generative model for MNIST images of digits, generate digits that satisfy horizontal symmetry"}
    \end{itemize}

    Directly learning a conditional model $p(\mathbf{x} \mid \mathbf{y})$ for any condition $\mathbf{y}$ is not feasible.


    \textbf{Q1}: Can we generate data from a pre-trained diffusion model while enforcing arbitrary
...
% \begin{itemize}
%     \item We develop a method for sampling conditioning on logical constraints using unconditional pre-trained score-based generative models, without additional training

%     \item The objective is to approximate the true conditional distribution, rather than just finding high likelihood samples
...
Objective: generate new samples from the distribution $p(\mathbf{x})$ that generated the data

\begin{enumerate}

    \item Define a diffusion process $p(\mathbf{x}_t | \mathbf{x}_0)$ that gradually adds noise to clean data points $\mathbf{x}_0$ until pure noise is reached (as a standard Gaussian).

    \item Learn the noise-dependent score: $\mathbf{s}(\mathbf{x}_t,t) := \nabla_{\mathbf{x}_t} \ln{p(\mathbf{x}_t)}$, for example, by \textbf{denoising score matching}.

    \item Run a sampling technique that uses the score to sample new points by reversing the diffusion process.

    \begin{figure}[htbp!] 
    \centering    
    \includegraphics[width=0.6\textwidth]{assets/images/passaggio_anno/sde.png}
    \end{figure}
...
% To sample from the target $p_c(\myvec{x}) \propto p(\myvec{x})e^{c(\myvec{x})}$, we need the conditional noise-dependent score $\nabla_{\myvec{x}_t} \ln{p_c(\myvec{x}_t)}$:
...
%     \item At $t=0$ (no noise $\rightarrow$ analytical form) $$\nabla_{\myvec{x}_0} \ln{p_c(\myvec{x}_0)} =\nabla_{\myvec{x}_0} \ln{p(\myvec{x}_0)} + \nabla_{\myvec{x}_0} c(\myvec{x}_0)$$
    
%     \item At $t=1$ (only noise $\rightarrow$ analytical form) $$\nabla_{\myvec{x}_1} \ln{p_c(\myvec{x}_1)} = \nabla_{\myvec{x}_1} \ln{p(\myvec{x}_1)}$$ %, since enough noise is added to make samples from $p_c(\myvec{x}_1)$ %distributed as the prior.
    
%     \item When $t \in (0,1)$ there is no general analytical form $\rightarrow$ approximation
...
We approximate the conditional score by interpolating the two regimes where the conditional score is known without approximation (no noise $t=0$, only noise $t=1$):
%We can obtain an approximation by interpolating the two regimes:
$$\tilde{\myvec{s}}_c(\myvec{x}_t, t) = \nabla_{\myvec{x}_t} \ln p(\myvec{x}_t) + g(t) \nabla_{\myvec{x}_t} c(\myvec{x}_t)$$
where $g(0) = 1$ and $g(1) = 0$.

We use as weighting function $g(t)$ the signal-to-noise ratio of the diffusion kernel.
    \begin{figure}
    \centering    
    \includegraphics[width=0.5\textwidth]{assets/images/g(t).pdf}
    \end{figure}
Intuition: the gradient of the constraint is more meaningful as the data gets less noisy.
...
\begin{itemize}
    \item The score approximation needs a differentiable constraint $\rightarrow$ we relax logical constraint into soft, differentiable constraints.
...
\frametitle{Neuro-Symbolic Logical Constraints}
    \begin{figure}
        \centering
        \includegraphics[width=0.7\textwidth]{assets/images/log_probabilistic_functions.pdf}
    \end{figure}
\end{frame} %-----------------------------------
...
% \frametitle{Training-Free Conditional Sampling}

% \textbf{Input}: pre-trained score-based diffusion model $\nabla_{\myvec{x}_t} \ln p(\myvec{x}_t)$ for $p(\myvec{x})$ and logical constraint $\pi$.
% \begin{enumerate}

%     \item Convert the logical constraint to a differentiable constraint using the neuro-symbolic framework:
%         \begin{equation}
%             \pi(\mathbf{x}) \rightarrow c(\mathbf{x})
%         \end{equation}
    
%     \item Approximate the conditional score
%         \begin{equation}
%     \tilde{\myvec{s}}_c(\myvec{x}_t, t) = \nabla_{\myvec{x}_t} \ln p(\myvec{x}_t) + g(t) \nabla_{\myvec{x}_t} c(\myvec{x}_t)
%         \end{equation}
    
%     \item Substitute the new score function in the original score-based sampling algorithm to generate new points

% \end{enumerate}
% %\textbf{Output}: approximated conditional samples from $p(\myvec{x} | \pi)$
...
\begin{itemize}
    \item \textbf{Data}:
    \begin{itemize}
        \item Tabular data: White wine (11 features) dataset and Adult (15 features)
        \item Time series: simulated eSIRS time series (spreading of a disease in an open population)
        \item Images: MNIST and CelebA
    \end{itemize}
    \item \textbf{Baseline}:
    \begin{itemize}
        \item Rejection Sampling (RS) as ground truth (when possible)
        \item Universal Guidance \citep{bansal2023universal}
    \end{itemize}
    \item \textbf{Metrics}:
    \begin{itemize}
        \item Total variation distance: $ D(X,Y):=\frac{1}{2} \sum_i |x_i - y_i|$ where $x_i$ and $y_i$ are the empirical probabilities for a given common binning of samples $X$ and $Y$. %This distance is upper bounded by 1. Intuitively, it is the area of each of the two histograms that is not shared.
        \item Average l1 distance in correlation coefficients.
        \item Satisfaction of the constraint (\% of samples).
    \end{itemize}
...
%     \item Adult (\cite{dataset_adult}): 5 numerical and 10 categorical dimensions.
% \end{itemize}

% Unconditional score-based models are trained using SDEs (\cite{sde}). Categorical variables were embedded in continuous space using one-hot encodings.
...
\begin{enumerate}
    \item \textbf{White wine dataset}: \texttt{$(\text{fixed acidity} \in [5.0, 6.0]  \lor  \text{fixed acidity} \in [8.0, 9.0]) \land \text{alcohol} \geq 11.0 \land (\text{residual sugar} \leq 5.0 \rightarrow \text{citric acid} \geq 0.5)$}
    \item \textbf{White wine dataset}: \texttt{$\text{alcohol}_1 > \text{alcohol}_2 + 1$}
    \item \textbf{Adult dataset}: \texttt{$\text{age} \geq 40 \land (\text{race} \neq \text{"White"} \lor \text{education} = \text{"Masters"})$}
\end{enumerate}
...
\textbf{Experiment} &
  ...
  \multicolumn{1}{c}{\textbf{RS acceptance rate}} \\ \midrule
White wine        & 0.1    & 0.07  & 86\%  & 1.67\% \\
White wine (multi) & 0.061  & 0.027 & 99\%   & 27\%   \\
Adult             & 0.067  & 0.046 & 100\% &  1.73\% \\ \bottomrule
\end{tabular}%
}
\end{table}

\begin{itemize}
    \item Low error for simple constraints, still acceptable for complex constraints.
    \item More efficient sampling in low probability regions.
\end{itemize}
...
\frametitle{Experiments: Images}
    ...
    \textbf{Image symmetry}:

    \begin{figure}[htbp!] 
    \centering    
    \includegraphics[width=0.45\textwidth]{assets/images/passaggio_anno/mnist_symmetry.png}
    \end{figure}

    \begin{figure}[htbp!] 
    \centering    
    \includegraphics[width=0.45\textwidth]{assets/images/passaggio_anno/celebA_symmetry.png}
    \end{figure}

    We had to trade-off sample quality and constraint satisfaction by tuning the constraint strength $k$.
...
\frametitle{Experiments: Images}
    ...
    \textbf{Image restoration} (with known corruption process $f$):
    $$\forall i \ f(\myvec{x})_i = \tilde{\myvec{y}}_i$$

    \begin{figure}[htbp!] 
    \centering    
    \includegraphics[width=0.6\textwidth]{assets/images/passaggio_anno/celebA_restoration.png}
    \end{figure}
...
\begin{frame} %-----------------------------------
\frametitle{Comparison with Universal Guidance \citep{bansal2023universal}}


        \begin{itemize}            
        \item Synthesis of previous optimization-based approaches, originally applied to images

        %\item It involves at each time step an optimization of the denoising prediction $\myvec{x}_0$, in order to make it satisfy the constraint $c(\myvec{x}_0)$.

        \item It fails in two of the tasks we tested (White wine and eSIRS bridging).
            
        %\item It performs better on one of the tasks based on images.
        \end{itemize}

        \begin{table}[ht]
\centering
%\caption{Comparison with Universal Guidance. On the white wine and eSIRS bridging tasks our method performs significantly better in terms of TVD (we report average and maximum distance over the marginals).}\label{tab:universal_guidance_comparison_short}
\begin{tabular}{@{}lcccc@{}}
\toprule
\multicolumn{1}{c}{\multirow{2}{*}{Method}} & \multicolumn{2}{c}{White wine} & \multicolumn{2}{c}{eSIRS bridging} \\ \cmidrule(l){2-5} 
\multicolumn{1}{c}{} & \textbf{Avg TVD} & \textbf{Max TVD} & \textbf{Avg TVD} & \textbf{Max TVD} \\ \midrule
\textbf{Ours}        & $\mathbf{0.1}$    & $\mathbf{0.15}$  & $\mathbf{0.13}$  & $\mathbf{0.25}$  \\
...
%         \begin{figure} 
%         \centering    
%         \includegraphics[width=0.9\textwidth]{assets/images/white_wine_fw_bw.pdf}
%         \end{figure}
...
\textbf{Contributions}
\begin{itemize}
    \item We developed a method for training-free conditional distribution approximation with logical constraints, that can be applied to any pre-trained score-based diffusion model.
    \item Our method performs well on training-free conditional distribution approximation on tabular data with logical constraints.
\end{itemize}

\textbf{Limitations}
\begin{itemize}
    \item As complexity increases, it is necessary to trade off constraint satisfaction with sample quality.

    \item Optimization-based methods are preferable when it is more important to obtain high-quality samples, rather than approximating the true conditional distribution.
