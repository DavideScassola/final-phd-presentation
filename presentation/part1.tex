...
\frametitle{Controlling Generative Models}
    ...
    A relevant problem in generative models is to efficiently achieve conditional generation forcing user defined logical constraints on generated samples.
...
We use as weighting function $g(t)$ the signal-to-(signal plus noise) ratio of the diffusion kernel:
    \begin{figure}
    \centering    
...
\begin{itemize}
    ...
    \item \textbf{Baseline}:
    \begin{itemize}
        \item Rejection Sampling (RS) when possible
...
\frametitle{Experiments: Images}
    ...
    We had to trade-off sample quality and constraint satisfaction by tuning the constraint strenght $k$.
...
\begin{columns}
    \column{0.5\textwidth}
        \begin{itemize}            
        \item Universal Guidance (\cite{bansal2023universal}) is a Synthesis of previous optimization-based approaches, originally applied to images

        \item It involves at each time step an optimization of the denoising prediction $\myvec{x}_0$ (that is a linear transformation of the score), in order to make it satisfy the constraint $c(\myvec{x}_0)$.

        \item Universal guidance fails in two of the tasks we tested (White wine and eSIRS bridging).
        
        \item However, it performs better on one of the tasks based on images.
        \end{itemize}
...
\begin{itemize}
    ...
    \item Our method is performs well on training-free conditional distribution approximation on tabular data with logical constraints.
\end{itemize}

\textbf{Limitations}
\begin{itemize}
    \item As complexity increases, it is necessary to trade-off constraint satisfaction with samples quality.

    \item Optimization-based methods are preferrable when it is more important to obtain high quality samples, rather than approximating the true conditional distribution.
