\section{Graph-Conditional Flow Matching for Relational Data Generation}

% -------------------------------------------------------------------------------------
\begin{frame}[fragile]{The Problem}
  \begin{columns}[T,onlytextwidth]
    \column{0.5\textwidth}
    \textbf{Q2}: Can we design expressive generative models capable of generating relational data with complex structural dependencies?
\bigskip
...
\begin{frame}[fragile]{Generating Relational Data is Hard}
  \begin{columns}[T,onlytextwidth]
    \column{0.4\textwidth}
    Generating relational data requires modeling statistical dependencies between records connected by foreign keys (also indirectly).
  \bigskip
...
\begin{frame}[fragile]{Graph-Conditional Flow Matching}
  ...
  \begin{itemize}
    \item \textbf{Expressive}: records in a connected component are modeled jointly
...
\begin{itemize}
  ...
  \item Categoricals are one-hot encoded, and the denoiser distribution is either categorical or Gaussian depending on the feature type
\end{itemize}
...
\textbf{Sampling}:
\begin{enumerate}
  \item Generate the foreign-key graph $G \sim p(G)$ (we just re-sample connected components from the original graph)
...
\end{table}
  ...
  \begin{itemize}
    \item We achieve state-of-the-art performance
    %\item Ablation shows importance of GNN for modeling dependencies
...
\begin{frame}[fragile]{Our Contribution}
  ...
  \begin{itemize}
    ...
    \item We achieve generalization by controlling ``within-sample'' generalization and apply a bottleneck on the GNN embedding size.
  \end{itemize}
  ...
  \begin{itemize}
    ...
    \item We avoid dealing with dense adjacency matrices.
